...
项目现在已经重构为使用LangGraph框架，将原有的线性处理流程转换为基于状态机的架构，使系统更加模块化、可扩展和可维护。最新的版本引入了动态规划分析流程，能够迭代优化分析结果，提供更准确的分析报告。系统现在包含增强的分析规划器，带有智能学习机制，以及观察和评估模块，可以根据结果质量决定是否需要重新规划。

项目现在完全依赖大模型在线生成代码来执行数据分析任务，实现了更智能的错误修复和跨工作表操作支持。
...
│   ├── enhanced_analysis_planner.py # 增强数据分析任务规划器(含智能学习)
...
4. **预设问题**：提供"多角度多维度分析"和"多维度交叉分析"两个预设问题按钮，点击后将文本填充到输入框中。
...
37. **条件路由机制**：根据用户请求内容自动选择合适的处理流程（分析流程或聊天流程）。
38. **智能列名匹配**：采用通用列匹配算法，提升多工作表数据处理的准确性。
39. **多维度数据透视**：支持分组统计、交叉表和透视表等多维度数据透视分析。
40. **质量阈值控制**：支持基于质量评分的提前终止机制，避免不必要的迭代。
41. **多工作表智能合并**：支持将多个工作表的数据合并为统一的数据集进行分析。
42. **完全依赖大模型生成代码**：数据处理器完全依赖大模型在线生成pandas代码来执行分析任务。
43. **Pandas代码执行与修复**：自动修复pandas代码中的常见错误，如元组列选择问题、语法错误等。
44. **跨工作表操作处理**：支持对多个工作表的数据进行交叉分析和比较。
45. **统一模型参数管理**：通过create_model_params函数统一管理模型配置参数。
46. **API参数验证与限制**：自动验证并限制参数值在API允许范围内。
47. **智能重规划决策**：基于质量评分和反馈决定是否需要重新规划分析任务。
48. **智能学习统计**：提供重规划周期等智能学习指标的统计信息。
49. **改进的流式响应处理**：优化SSE流处理和数据展示性能。
50. **数据类型转换**：自动将pandas和numpy数据类型转换为Python原生类型以支持JSON序列化。
51. **客户留存分析**：支持跨工作表客户留存、新增和流失分析。
52. **交叉表分析增强**：改进的交叉表功能，支持更复杂的关联性分析。
53. **语法错误自动修复**：智能修复生成代码中的语法错误、缩进错误等。
54. **多工作表数据合并**：支持多个工作表数据的智能合并与关联分析。
55. **列名智能映射**：自动处理不同工作表间的列名映射与前缀处理。
56. **GroupBy操作支持**：支持复杂的分组统计操作，包括多列分组。
57. **按位操作错误修复**：自动处理因NaN值导致的按位取反操作错误。
...
63. **智能采样机制**：实现CSV/XLSX文件的智能采样机制，保留所有列名并随机采样最多50行数据。
...
或者在配置页面中直接设置 API 密钥，也可以使用 `set_key.sh` 脚本设置API密钥。
...
- 实现改进的历史学习机制，优化重规划决策
- 实现聊天历史压缩功能，当历史记录过长时使用大模型智能压缩
- 实现聊天历史压缩器的集成，支持token估算和压缩功能
- 支持跨工作表数据操作处理
- 改进数据类型转换，确保所有pandas/numpy数据类型都能正确序列化
- 实现原始文件内容的完整保存和处理（original_file_content字段）
- 优化大模型流式API调用，实现真正的逐字输出体验
- 实现文件内容的智能截断机制，避免超出上下文限制
- 实现CSV/XLSX文件的智能采样机制，保留所有列名并随机采样最多50行数据
- 改进文件上传API，返回采样内容用于任务规划和文件ID用于访问完整原始内容
- 实现服务器端临时文件存储机制，支持完整原始文件内容的数据处理
- 实现临时文件自动清理机制，定期清理过期文件
- 实现上传文件的智能采样功能，保留所有列名并随机采样最多50行数据
- 实现基于文件ID的完整文件内容访问机制
- 实现聊天历史压缩功能的集成和调用
- 实现动态规划分析流程的改进和优化
- 实现质量评估和迭代控制机制的集成
...
- 实现提前终止机制，当质量评分满足阈值时提前结束迭代
- 实现改进的动态规划分析流程，优化迭代控制逻辑
- 实现条件路由逻辑，根据用户请求内容选择合适的处理流程
- 延迟初始化图实例以避免循环导入
- 支持跨工作表数据分析流程的集成
- 改进质量阈值和迭代控制的实现逻辑
- 优化观察和评估节点的决策机制
...
- 实现质量阈值检查，避免不必要的重规划
- 实现迭代计数管理，确保不超过最大迭代次数
- 实现详细的重规划请求构建，包含具体的评估反馈
- 实现质量阈值控制，支持提前终止重规划流程
- 实现跨工作表数据分析的节点处理逻辑
- 优化数据处理结果的序列化和转换
...
- 实现历史规划学习机制，从之前的迭代中提取改进策略
- 实现智能初始规划，基于用户请求和历史记录优化规划质量
- 解析文件内容以获取实际列名
- 实现多维度交叉分析和多工作表数据分析的支持
- 支持跨工作表数据关联分析的规划
- 优化列名解析和映射机制
- 改进多工作表分析任务的规划逻辑
...
- 实现业务价值导向的评估机制，重点关注业务洞察质量
- 实现智能决策逻辑，根据质量评分和反馈决定后续操作
- 实现JSON响应解析和错误处理
- 实现质量阈值控制机制
- 支持跨工作表分析结果的评估
- 优化评估提示词，提高评估准确性
...
- 实现基于大模型的智能摘要策略
- 支持自定义token限制和保留比例
- 实现更精确的token估算方法，区分中英文字符
- 实现安全边界和保留比例控制
- 实现大模型摘要回退策略
- 优化token估算准确性，支持更复杂的文本结构
- 实现改进的摘要生成逻辑，使用settings参数进行模型配置
...
- 支持管道符分隔的数据处理
- 实现多工作表数据合并功能
- 实现更高级的列匹配逻辑，支持语义相似性匹配
- 实现对多工作表数据的交叉分析支持
- **完全依赖大模型在线生成代码**来执行数据分析任务
- 实现pandas代码的执行与错误修复功能
- 实现pandas和numpy数据类型的JSON序列化转换
- 实现跨工作表操作的特殊处理
- 实现列名映射和前缀处理机制
- 实现客户留存分析等高级跨工作表操作
- 支持复杂的GroupBy和聚合操作
- 实现语法错误自动修复功能
- 支持DataFrame操作的智能转换（如元组转列表）
- 实现按位操作错误修复（处理NaN值问题）
- 优化代码执行环境，提高执行成功率
- 改进多工作表数据合并与关联机制
- 增强交叉表和透视表功能的实现
...
- 实现流式和非流式两种响应模式
- 包含详细的日志记录功能
- 支持settings参数统一管理模型配置
- 实现 `create_model_params` 函数统一管理模型参数
- 实现API参数验证与范围限制
- 实现错误处理和HTTP错误处理机制
- 实现流式响应处理和内容生成
- 实现请求载荷准备和消息格式化
...
- 生成专业的业务数据透视分析报告
- 接收和使用settings参数，统一管理模型配置
- 结合业务洞察和数据分析结果生成综合性报告
- 支持自定义报告结构和内容格式
- 实现业务数据透视和洞察分析的专业报告生成
- 优化报告生成的提示词，提高报告质量
- 支持跨工作表分析结果的报告生成
- 增强业务洞察分析的专业性
- 改进表格格式输出的结构和内容
...
- 实现改进的用户界面样式和视觉效果
- 支持Word导出按钮的样式
- **统一的配色方案**：使用CSS变量实现基于设计配色方案的颜色系统，包括主色系、功能色、中性色、背景色和侧边栏颜色
...
- **请求体**：
  ```json
  {
    ...
    "file_id": "文件ID，用于访问服务器上存储的完整原始文件内容（可选）",
    "history": "聊天历史记录（可选）",
...
- **响应**：
  - `file_content`：采样后的文件内容（用于任务规划，CSV/XLSX文件会进行智能采样，保留所有列名并随机采样最多50行数据）
  - `file_id`：文件ID（用于访问服务器上存储的完整原始文件内容）
  - `filename`：上传的文件名
...
- `original_file_content`: 保存完整的原始文件内容用于数据处理
- `chat_history`: 聊天历史记录
- `settings`: AI模型配置参数
...
> **📖 设计配色方案**：前端设计必须遵循项目统一的设计配色方案，详见 [`.iflow/commands/design_colors.md`](.iflow/commands/design_colors.md)。

**前端设计必须遵守以下配色原则：**

- **主色系**：使用蓝色系（`#3B82F6`）作为主色调，保持专业感
- **功能色**：成功（青绿 `#108981`）、警告（橙 `#F59E0B`）、危险（红 `#EF4444`）清晰区分
- **中性色**：统一使用定义的中性色体系，提高整体一致性
- **品牌识别**：品牌红（`#8B1919`）用于重要品牌标识和强调
- **可访问性**：确保文字与背景有足够对比度（WCAG AA 标准）
- **色彩一致性**：所有前端组件必须使用统一的配色方案，不得随意使用未定义的颜色

**使用方式：**
- 在 CSS 中使用 CSS 变量（如 `var(--primary)`、`var(--success)` 等）
- 参考配色方案文档中的具体应用示例（按钮、状态标签、卡片、表格等）
- 新增前端组件时，必须先查阅配色方案文档，确保颜色使用符合规范
...
- 正确处理步骤4及最终报告的显示逻辑
- 引入了智能学习机制，从历史规划中学习以减少不必要的重规划
- 实现了智能学习机制，从历史规划中学习以减少重新规划
- 支持质量评估和反馈循环，持续改进分析结果
- 实现了改进的重规划节点，使用历史学习优化规划质量
...
- 实现条件路由机制，根据用户请求内容自动选择合适的处理流程
- 实现多工作表数据的智能合并与分析
- 实现基于质量评分的提前终止机制，提高系统效率
- **完全依赖大模型在线生成pandas代码**来执行数据分析任务
- **实现pandas代码执行与错误修复**功能，自动处理常见错误
- **实现跨工作表数据操作支持**，处理多工作表间的关联分析
- **实现统一的模型参数管理**，通过create_model_params函数统一处理
- **实现API参数验证与范围限制**，确保参数在API允许范围内
- **实现改进的流式响应处理**，优化SSE流处理性能
- **实现pandas和numpy数据类型的JSON序列化转换**，确保数据可传输
- **实现智能重规划决策**，基于质量评分和反馈决定是否需要重新规划
- **实现智能学习统计**，跟踪重规划周期等学习指标
- **实现聊天历史压缩器集成**，支持更高效的对话管理
- **实现数据透视和业务洞察分析**，从商业角度提供专业分析
- **实现改进的迭代控制逻辑**，支持最多5轮迭代优化
- **实现客户留存分析功能**，支持跨时间点客户分析
- **实现语法错误自动修复**，提高代码执行成功率
- **实现更智能的列名映射机制**，优化多工作表数据处理
- **实现改进的GroupBy操作支持**，处理复杂的分组统计需求
- **实现按位操作错误处理**，避免NaN值导致的错误
- **实现多维度交叉分析支持**，提供更深入的数据洞察
- **实现DataFrame操作智能转换**，自动修复元组列选择问题
- **实现业务指标深度分析**，提供更专业的商业洞察
- **实现原始文件内容的完整处理**，确保数据完整性
